...
struct MC_data
{
    ...
    char e_type;    // e for exact,  E for Euler (evolution type)
...
double Next_S(double S, double drift, double drift_r, double sgrt, char e_type);
double Next_S_exact(double S, double drift, double sgrt);
double Next_S_Euler(double S, double drift_r, double sgrt);
...
{
    ut::OutputLine("Monte Carlo version 1:  uses functions,  project,  1");
    ...
    for(long j = 1; j <= dat.M; ++j)
    {
        ...
        {
            S = Next_S(S, drift, drift_r, sgrt, dat.e_type);
...
void ReadInData(MC_data & dat)
{
	...
	dat.o_type = 'p';    // p for plain,  a for average rate, (option type)
						 // l for lookback, b for barrier
    ...
    dat.e_type = 'E';    // e for exact,  E for Euler  (evolution type)

    dat.M = 10000000;  // number of sample paths
    dat.N = 5;     // number of time steps
}
...
bool Validate(MC_data & dat)
{
	...
	if (dat.k_type !='i' && dat.k_type !='o')
    {
		ut::OutputLine("Bad knock type character", dat.k_type);
		return false;
	}

	if (dat.e_type !='e' && dat.e_type !='E')
    {
		ut::OutputLine("Bad evolution type character", dat.e_type);
		return false;
	}
...
double Next_S(double S, double drift, double drift_r, double sgrt, char e_type)
{
    ...
    switch(e_type)
    {
        case 'e': S_val = Next_S_exact(S, drift, sgrt);
                  break;

        case 'E': S_val = Next_S_Euler(S, drift_r, sgrt);
                  break;
    }
    return S_val;
}
